...
function getAge(birthday) {
    ...
    var ageDifMs = fechaActual - birthday;
    var ageDate = new Date(ageDifMs); // miliseconds from epoch
    var age = Math.abs(ageDate.getUTCFullYear() - 1970);
    
    //Validacion
    //Si el mes todavia no ha pasado
    if(ageDate.getMonth > fechaActual.getMonth){
        return (age - 1);
    }
    //Si es el mismo mes pero el dia todavia no ha pasado
    else if((ageDate.getMonth == fechaActual.getMonth && ageDate.getDate > fechaActual.getDate)){
        return (age - 1);
    }
    //Si ya paso
    else{
